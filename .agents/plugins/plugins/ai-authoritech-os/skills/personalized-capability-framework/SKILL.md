...
## Required inputs
Request, business/brand, intended use, target audience, output dimensions, available references, non-negotiable elements, and delivery format. Infer missing preferences only from approved memory or locked assets and label every inference.

## Procedure
1. Classify the request as new generation, edit, continuity, campaign, product, technical, printable, 3D reference, or workflow design.
2. Resolve the active brand and prevent cross-brand style leakage.
3. Identify locks: identity, style, world, product geometry, logo, typography, layout, colors, and text.
4. Select the smallest complete skill chain from the routing guide.
...
## Example
A branded social visual routes through Prompt Architect, Brand Visual Translator, the appropriate style director, Composition, Revision QA, and Governance.

## Future expansion
Add performance scoring by model, automated preference extraction, and reusable brand/character lock packages.
