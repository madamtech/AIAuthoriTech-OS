...
- `discovered/` contains non-authoritative records for GPTs referenced in conversation, notes, screenshots, bookmarks, or other evidence but not yet captured from the live Builder. Authorization and configuration completeness are tracked separately; authorization to reuse a GPT does not imply its Builder configuration has been captured.
